...
import { useAppMutation, useCollections, useNotes, useRunGroups, useRuns, useVersionContent, useVersions } from "../hooks/use-data";
import {
...
function MoveToCollectionDialog({
  ...
  if (!open) return null;
  return (
    <div className="pb-overlay fixed inset-0 z-40 flex items-center justify-center bg-black/60" onClick={() => onOpenChange(false)}>
      <div
      ...
      >
        ...
        <div className="max-h-64 space-y-0.5 overflow-y-auto">
          {(collections ?? []).length === 0 && (
            <p className="text-[12px] text-ink-faint">No collections yet — create one from the left rail.</p>
          )}
          {(collections ?? []).map((collection) => {
            ...
            return (
              ...
              >
                ...
                />
                {collection.name}
                <span className="ml-auto text-[11px] tabular-nums text-ink-faint">{collection.promptCount}</span>
              </label>
            );
          })}
        </div>
